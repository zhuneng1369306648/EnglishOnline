									...
									<tr>
										...
										<c:if test="${item.remark == null}">
											...
											<c:if test="${item.course.period == item.progress}">
												<td>未推荐</td>
												<td>
													<button class="btn btn-default btn-xs btn-info" onClick="location.href='${ctx}/teacher/remark?studentId=${item.studentId}&courseId=${item.courseId}'">推荐</button>
												</td>
											</c:if>
										</c:if>
										<c:if test="${item.remark != null}">
